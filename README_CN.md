...
1. 前往 [下载页面](https://github.com/HCLonely/TrayPilot/releases)，选择 **完整版（full，推荐）**；如果已安装 .NET 10 Desktop Runtime x64，也可以选择体积更小的 **精简版（lite）**。
...
- **全部恢复**：仅恢复本工具隐藏的普通软件托盘图标，保留隐藏规则，不改变系统图标的状态和隐藏选择。
- **彻底取消规则**：在 **隐藏规则** 中删除对应项目。
- **退出程序**：默认直接退出，不主动恢复隐藏图标；勾选设置中的 **退出后恢复图标** 后，退出时才恢复。隐藏规则和未清理的恢复记录保留供下次使用。
...
- 选择 **退出后恢复图标**（默认关闭），同时控制普通托盘图标和系统图标的退出恢复行为；关机或注销时也遵循此设置。
...
系统图标的选择与普通软件规则分开保存。主界面的 **全部恢复**只恢复普通软件托盘图标；**系统图标** 窗口中的 **全部恢复**只恢复系统控件，并清除系统图标的隐藏选择，不影响普通软件图标或规则。**显示所有**仍会清除系统图标的隐藏选择；需要再次隐藏时，请重新设置。
