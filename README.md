...
| 全部恢复（主界面按钮） | 恢复本工具隐藏的图标；恢复成功后清除所有隐藏规则 |
| 退出 | 恢复本工具隐藏的图标并退出，保留隐藏规则供下次启动使用 |

**显示所有与全部恢复不同：前者保留规则，后者清除规则。** 自动隐藏的暂停状态会保存并跨重启生效；手动恢复产生的临时例外不会保存，执行 **隐藏规则命中** 也会清除这些例外。
...
**语言** 支持简体中文（默认）和 English，保存后立即切换。语言、主题、关闭到托盘、自身图标显示状态、自动隐藏暂停状态及快捷键均会保存。软件名称、路径和第三方提示文字保持原文。
...
键使用简短、稳定的语义化英文标识，采用 `camelCase` 命名（如 `mainWindowTitle`、`trayIconDetails`），不包含显示文案中的标点、换行或格式占位符。复制现有完整语言包，只翻译 `Strings` 的值，保留键和 `{0}`、`{1}` 等格式占位符。文件名（不含扩展名）作为语言标识，`Name` 为设置中的显示名称。重新打开设置即可发现新增文件。缺失翻译回退为中文；缺失或无效的语言包不会阻止正常图标管理。
