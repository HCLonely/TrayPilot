...
**Current version: 0.5.4** · [简体中文](README.md) | English
...
3. Double-click an application icon or row to show or hide its icons. For persistent automatic hiding, right-click and choose **Add to matching rules**.
...
Keep the `languages` folder next to the executable when moving or distributing it. `app/` is a local publish output directory and is not committed with the source.
...
| Left double-click | Toggle tray icons for all instances sharing the same executable path |
...
Rules match full executable paths, ignoring case and path-separator differences. All instances and icons sharing the same path are controlled together.
...
- A check mark means all tray icons for that application are shown. Click to hide them when all are shown, or show them all when any are hidden.
